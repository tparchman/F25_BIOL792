...
**2).** There are some problematic special characters in *some* of these files. To guard against them, you will need some regular expressions or string functions to "find and replace" some gremlins. Because you dont know where these will be, you will want to scrub each line in each file (or the file in its entirety. 

Code as below will work for cleaning these things out.

```py
    cleandataline = dataline.replace("\0","")
    cleandataline = re.sub("\0","", dataline) #alternative using re
```